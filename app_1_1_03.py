...
def case_open_callback(c):
    try:
        ...
        bot.answer_callback_query(c.id)
        if ok and card:
            send_card_photo(c.message.chat.id, card, msg)
        else:
            bot.send_message(c.message.chat.id, msg, parse_mode="HTML")
        if ok:
...
def render_character_list(chat_id, uid, username=None, first_name=None):
    ...
    _char_ui_replace(chat_id, uid, lambda: bot.send_message(chat_id, text, reply_markup=kb, parse_mode="HTML"))

def render_character_card(chat_id, uid, card_id):
    u = users.find_one({"_id": uid}) or {}
    card = next((x for x in u.get("cards", []) if x["id"] == card_id), None)
    if not card:
        return render_character_list(chat_id, uid)
    ...
    _char_ui_replace(chat_id, uid, lambda: send_card_photo(chat_id, card, caption, reply_markup=kb))
...
def sendcard_callback(c):
    try:
        ...
        send_card_photo(c.message.chat.id, card, f"🎁 Карта <b>{name}</b> передана!")
        try:
...
def send_turn_message(bid, action_text=""):
    """
    Всегда РЕДАКТИРУЕТ одно и то же сообщение боя (fight['message_id']),
    вместо отправки нового сообщения на каждое действие — чтобы бот не
    заваливал чат сообщениями.
    """
    fight = active_fights.get(bid)
    ...
    if msg_id:
        try:
            bot.edit_message_text(text, fight["chat_id"], msg_id, reply_markup=kb, parse_mode="HTML")
        except Exception as e:
            logger.warning(f"Не удалось отредактировать сообщение боя ({e}), отправляю новое")
...
def finish_fight(bid, winner_uid, loser_uid, reason=None):
    ...
    if msg_id:
        try:
            bot.edit_message_text(text, fight["chat_id"], msg_id, parse_mode="HTML")
        except Exception as e:
            logger.warning(f"Не удалось отредактировать финальное сообщение боя ({e}), отправляю новое")
