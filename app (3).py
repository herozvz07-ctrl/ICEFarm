...
def character_menu(m):
    ...
    try:
        ...
        bot.send_message(m.chat.id, f"🧬 <b>Character</b> — слотов {len(cards)}/{CARD_MAX_STORAGE}", parse_mode="HTML")

        for card in cards:
            name, hp, dmg, defp, rng = card_stats(card)
            active_mark = "⭐ " if card["id"] == active_id else ""
            caption = f"{active_mark}<b>{name}</b>\nHP: {hp} | DMG: {dmg} | DEF: {defp}%"
            kb = types.InlineKeyboardMarkup(row_width=1)
            kb.add(types.InlineKeyboardButton("📋 Подробнее", callback_data=f"card_view_{card['id']}"))
            send_card_photo(m.chat.id, card, caption, reply_markup=kb)
    except Exception as e:
...
def card_view_callback(c):
    try:
        ...
        send_card_photo(c.message.chat.id, card, text, reply_markup=kb)
        bot.answer_callback_query(c.id)
    except Exception as e:
