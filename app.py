...
# --- НОВОЕ: ранг сжигания теперь даёт реальный бонус к фарму (а не просто картинку) ---
BURN_RANK_PERKS = {
    "":   0.0,    # 🧊 Лёд
    "🔥": 0.03,   # Горящий      +3% к фарму ICE
    "💀": 0.06,   # Пепел        +6% к фарму ICE
    "☄️": 0.10,  # Метеор       +10% к фарму ICE
    "🌋": 0.15,  # Вулкан       +15% к фарму ICE
}

def get_burn_rank_bonus(total_burned: float):
    """Возвращает постоянный % бонуса к фарму ICE за текущий ранг сжигания."""
    _, emoji = get_burn_rank(total_burned)
    return BURN_RANK_PERKS.get(emoji, 0.0)

# ================================================================
# НОВОЕ: КИРКИ — предметы, находимые во время фарма
# ================================================================

FARM_VIP_GIF = "https://i.ibb.co/N2pfn858/PIXELIAN.gif"   # гиф фарма только для VIP
...
def roll_farm_find(uid, inv):
    ...
    for key, chance in PICK_FIND_CHANCES.items():
        if random.random() < chance:
            ...
            inv.append(item)
            found_lines.append(f"🎉 Находка во время фарма: {PICK_DATA[key]['emoji']} <b>{PICK_DATA[key]['name']}</b>!")
    if random.random() < ICE_PILE_CHANCE:
        pile = round(random.uniform(ICE_PILE_MIN, ICE_PILE_MAX), 1)
        bonus_ice += pile
        found_lines.append(f"🎉 Находка во время фарма: кучка льда +<b>{fmt(pile)} ICE</b>!")
...
def farm(m):
    """
    НОВОЕ: одна кнопка запускает СРАЗУ оба фарма — ICE и Snow Jewel (капсула).
    В одном сообщении показываются оба результата, VIP-гифка (только для VIP),
    бонусы от предметов (кирок) и от ранга сжигания, находки во время фарма.
    """
    ...
    try:
        ...
        if ice_ready:
            ...
            item_bonus_pct, item_bonus_lines = get_pick_bonus_pct(inv)
            if item_bonus_pct:
                gain += round(base_gain * item_bonus_pct, 2)

            burn_bonus_pct = get_burn_rank_bonus(u.get("total_burned", 0.0))
            if burn_bonus_pct:
                gain += round(base_gain * burn_bonus_pct, 2)
            ...
            if is_vip:
                ...
                gain += vip_bonus
                vip_line = f"✨ (VIP Бонус +{int(VIP_FARM_BONUS_PCT*100)}% от основного фарма = +{fmt(vip_bonus)} ICE)"

            found_lines, bonus_ice = roll_farm_find(u["_id"], inv)
            gain = round(gain + bonus_ice, 2)

            final_balance = round(final_balance + gain, 2)
            ...
            lines.append(f"❄️ Вы добыли <b>{fmt(gain)} ICE</b>")
            if vip_line:
                lines.append(vip_line)
            lines.extend(item_bonus_lines)
            if burn_bonus_pct:
                rank_name, rank_emoji = get_burn_rank(u.get("total_burned", 0.0))
                lines.append(f"{rank_emoji} (Бонус ранга «{rank_name}»: +{int(burn_bonus_pct*100)}%)")
            lines.extend(found_lines)
            lines.append(f"💰 Баланс: <b>{fmt(final_balance)} ICE</b>")
        else:
            wait = effective_cd - time_passed
            hours, minutes = wait // 3600, (wait % 3600) // 60
            lines.append(f"⏳ ICE-фарм: следующий через <b>{hours}ч {minutes}м</b>")

        lines.append("")  # разделитель между фармами

        # ---------- 2) SNOW JEWEL ФАРМ (капсула) ----------
        cap = u.get("capsule", {"owned": False, "level": 0, "last_farm": 0})
        if not cap.get("owned"):
            lines.append(f"{SNOW_EMOJI} [ Оборудование ещё не куплено — купите капсулу в меню «💊 Капсула» ]")
        else:
            ...
            if cap_left <= 0:
                users.update_one(
                    {"_id": u["_id"]},
                    {"$inc": {"snow_jewels": yield_amount}, "$set": {"capsule.last_farm": now}}
                )
                check_achievements(u["_id"])
                lines.append(f"{SNOW_EMOJI} Вы добыли <b>{yield_amount} {SNOW_EMOJI}</b> (капсула)")
            else:
                h, mnt = cap_left // 3600, (cap_left % 3600) // 60
                lines.append(f"{SNOW_EMOJI} Капсула: следующий сбор через <b>{h}ч {mnt}м</b>")
        ...
        if is_vip:
            try:
                bot.send_animation(m.chat.id, FARM_VIP_GIF, caption=text, parse_mode="HTML", message_thread_id=t_id)
                return
            except Exception as e:
                logger.warning(f"Не удалось отправить VIP-гиф фарма ({e}), отправляю обычным сообщением")
...
def view_nft_callback(c):
    try:
        ...
        if index < len(inv):
            ...
            if nft.get("type") == "item":
                # НОВОЕ: кирки и прочие текстовые предметы (без file_id/фото)
...
def burn_coins(m):
    ...
    if len(parts) < 2:
        my_bonus = get_burn_rank_bonus(burned)
        my_bonus_line = f"\n🎁 Ваш бонус к фарму ICE: <b>+{int(my_bonus*100)}%</b> (постоянно)" if my_bonus else ""
        # НОВОЕ: у каждого ранга теперь реальный бонус к фарму, а не только картинка/название
        bot.send_message(
            m.chat.id,
            ...
            f"{my_bonus_line}"
            f"{next_rank_text}\n\n"
            f"<b>Ранги сжигания (дают постоянный бонус к фарму ICE!):</b>\n"
            f"🧊 Лёд — 0 ICE (без бонуса)\n"
            f"🔥 Горящий — 100 ICE (+{int(BURN_RANK_PERKS['🔥']*100)}% к фарму)\n"
            f"💀 Пепел — 500 ICE (+{int(BURN_RANK_PERKS['💀']*100)}% к фарму)\n"
            f"☄️ Метеор — 1 000 ICE (+{int(BURN_RANK_PERKS['☄️']*100)}% к фарму)\n"
            f"🌋 Вулкан — 5 000 ICE (+{int(BURN_RANK_PERKS['🌋']*100)}% к фарму)\n\n"
...
def burn_callback(c):
    ...
    if old_rank != new_rank:
        new_bonus = get_burn_rank_bonus(new_burned)
        bonus_line = f"\n🎁 Бонус к фарму ICE: <b>+{int(new_bonus*100)}%</b> (постоянно)" if new_bonus else ""
        rank_up_text = f"\n\n🎉 <b>Новый ранг: {new_rank} {new_emoji}</b>{bonus_line}"
